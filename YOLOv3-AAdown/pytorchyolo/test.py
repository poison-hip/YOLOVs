...
from pytorchyolo.utils.transforms import DEFAULT_TRANSFORMS, RECTANGULAR_TRANSFORMS
...
def _evaluate(model, dataloader, class_names, img_size, iou_thres, conf_thres, nms_thres, verbose, max_det=300, nms_type="nms"):
    """Evaluate model on validation dataset.
    ...
    for _, imgs, targets in tqdm.tqdm(dataloader, desc="Validating"):
        ...
        with torch.no_grad():
            outputs = model(imgs)
            outputs = non_max_suppression(outputs, conf_thres=conf_thres, iou_thres=nms_thres, max_det=max_det, nms_type=nms_type)
...
def _create_validation_data_loader(img_path, batch_size, img_size, n_cpu):
    ...
    :rtype: DataLoader
    """
    transform = RECTANGULAR_TRANSFORMS if isinstance(img_size, (tuple, list)) and img_size[0] != img_size[1] else DEFAULT_TRANSFORMS
    dataset = ListDataset(img_path, img_size=img_size, multiscale=False, transform=transform)
