...
def rescale_boxes(boxes, current_dim, original_shape):
    ...
    if isinstance(current_dim, (tuple, list)):
        # Rectangular resize path used by 256x128 training/inference.
        # This branch assumes direct resize without square letterbox padding.
        resized_h, resized_w = current_dim
        boxes[:, 0] = boxes[:, 0] / resized_w * orig_w
        boxes[:, 1] = boxes[:, 1] / resized_h * orig_h
        boxes[:, 2] = boxes[:, 2] / resized_w * orig_w
        boxes[:, 3] = boxes[:, 3] / resized_h * orig_h
        return boxes

    # The amount of padding that was added in square letterbox mode
...
def _bbox_diou(box1, box2, eps=1e-9):
    inter_lt = torch.max(box1[:, None, :2], box2[:, :2])
    inter_rb = torch.min(box1[:, None, 2:], box2[:, 2:])
    ...
    c_lt = torch.min(box1[:, None, :2], box2[:, :2])
    c_rb = torch.max(box1[:, None, 2:], box2[:, 2:])
    c_wh = (c_rb - c_lt).clamp(min=0)
    c2 = c_wh[..., 0] ** 2 + c_wh[..., 1] ** 2 + eps

    center1 = (box1[:, None, :2] + box1[:, None, 2:]) / 2
    center2 = (box2[:, :2] + box2[:, 2:]) / 2
    rho2 = ((center1 - center2) ** 2).sum(-1)
    return iou - rho2 / c2


def _diou_nms(boxes, scores, iou_thres, max_det):
    # Greedy NMS variant using DIoU criterion instead of plain IoU.
    ...
    while order.numel() > 0:
        ...
        if len(keep) >= max_det or order.numel() == 1:
            break
        rest = order[1:]
        dious = _bbox_diou(boxes[i].unsqueeze(0), boxes[rest]).squeeze(0)
        order = rest[dious <= iou_thres]
    return torch.tensor(keep, device=boxes.device, dtype=torch.long)


def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, max_det=300, nms_type="nms"):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # nms_type == "diou" is useful for crowded/small-object scenes.
        if nms_type == "diou":
            i = _diou_nms(boxes, scores, iou_thres, max_det=max_det)
        else:
