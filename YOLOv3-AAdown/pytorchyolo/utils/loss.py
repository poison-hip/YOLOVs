...
def compute_loss(predictions, targets, model):
    ...
    BCEobj = nn.BCEWithLogitsLoss(
        pos_weight=torch.tensor([1.0], device=device),
        reduction="none")
    # Optional objectness focal loss controls from cfg.
    focal_gamma = float(model.hyperparams.get("obj_focal_gamma", 0.0))
    focal_alpha = float(model.hyperparams.get("obj_focal_alpha", 0.25))
    # Optional small-object box loss reweighting controls from cfg.
    small_obj_power = float(model.hyperparams.get("small_obj_power", 0.0))
    small_obj_max_scale = float(model.hyperparams.get("small_obj_max_scale", 3.0))
    ...
    for layer_index, layer_predictions in enumerate(predictions):
        ...
        if num_targets:
            ...
            if small_obj_power > 0.0:
                # Emphasize smaller targets; clamp to avoid exploding gradients.
                area = (tbox[layer_index][:, 2] * tbox[layer_index][:, 3]).clamp(min=1e-6)
                box_scale = torch.clamp(area.pow(-small_obj_power), max=small_obj_max_scale)
            else:
                box_scale = 1.0
            # We want to minimize our loss so we and the best possible IoU is 1 so we take 1 - IoU and reduce it with a mean
            lbox += ((1.0 - iou) * box_scale).mean()  # iou loss
        ...
        obj_loss = BCEobj(layer_predictions[..., 4], tobj)
        if focal_gamma > 0.0:
            # Focal modulation on objectness to focus training on hard examples.
            prob = torch.sigmoid(layer_predictions[..., 4])
            p_t = tobj * prob + (1.0 - tobj) * (1.0 - prob)
            alpha_t = tobj * focal_alpha + (1.0 - tobj) * (1.0 - focal_alpha)
            modulating = (1.0 - p_t).pow(focal_gamma)
            obj_loss = obj_loss * alpha_t * modulating
        lobj += obj_loss.mean() # obj loss
...
def build_targets(p, targets, model):
    ...
    # Anchor matching threshold (ratio-based), configurable via cfg.
    anchor_t = float(model.hyperparams.get("anchor_t", 4.0))
    ...
    for i, yolo_layer in enumerate(model.yolo_layers):
        ...
        if nt:
            ...
            j = torch.max(r, 1. / r).max(2)[0] < anchor_t
