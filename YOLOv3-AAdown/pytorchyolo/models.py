...
class ECALayer(nn.Module):
    ...
    Used in shallow backbone stages to improve feature selectivity
    with very small compute overhead.
    """
...
class AADownsample(nn.Module):
    """Anti-aliased downsampling using avgpool + CBL.

    Replaces stride-2 conv in early stages to preserve details and reduce aliasing.
    """

    def __init__(self, in_channels: int, out_channels: int, conv_kernel_size: int = 1, stride: int = 2, activation: str = "leaky"):
        super().__init__()
        padding = (conv_kernel_size - 1) // 2
        self.pool = nn.AvgPool2d(kernel_size=stride, stride=stride, ceil_mode=False)
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=conv_kernel_size, stride=1, padding=padding, bias=False)
...
def create_modules(module_defs: List[dict]) -> Tuple[dict, nn.ModuleList]:
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            pad = (kernel_size - 1) // 2
            # Optional grouped/depthwise conv support from cfg.
        ...
        elif module_def["type"] == "eca":
            # Lightweight channel attention block (no shape change).
            kernel_size = int(module_def.get("size", 3))
            modules.add_module(f"eca_{module_i}", ECALayer(output_filters[-1], kernel_size=kernel_size))
            filters = output_filters[-1]

        elif module_def["type"] == "aa_down":
            # Anti-aliased downsample block: AvgPool(stride=2) + Conv-BN-Act.
            filters = int(module_def["filters"])
            conv_kernel = int(module_def.get("size", 1))
            stride = int(module_def.get("stride", 2))
            activation = module_def.get("activation", "leaky")
            modules.add_module(
                f"aa_down_{module_i}",
                AADownsample(
                    in_channels=output_filters[-1],
...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool", "eca", "aa_down"]:
                x = module(x)
            elif module_def["type"] == "route":
