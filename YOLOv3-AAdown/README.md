...
## Embedded 256x128 Small-Object Variant

This repo now includes an embedded-oriented config at `config/yolov3-embedded-256x128.cfg` with:
- 3 detection heads on strides `16/8/4` (with a dedicated P2 head for small objects)
- anti-aliased downsampling blocks (`aa_down`)
- shallow ECA attention (`eca`)
- lightweight PAN-style bottom-up feature fusion
- depthwise-style grouped convs in detection heads

Example training:

```bash
poetry run yolo-train \
  --model config/yolov3-embedded-256x128.cfg \
...
Example inference at non-square input:

```bash
poetry run yolo-detect \
  --model config/yolov3-embedded-256x128.cfg \
  --weights checkpoints/yolov3_ckpt_XX.pth \
  --images data/samples \
  --img_width 256 \
  --img_height 128 \
  --conf_thres 0.12 \
...
### Auto-anchor clustering for 256x128

Use IoU k-means on your own labels:

```bash
python tools/kmeans_anchors.py \
  --train_list data/custom/train.txt \
  --img_width 256 \
  --img_height 128
```

Then copy the printed `anchors = ...` line into all `[yolo]` blocks in your cfg.
